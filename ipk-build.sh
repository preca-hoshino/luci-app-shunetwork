#!/bin/bash
# Build luci-app-shucampus .ipk using WSL/Linux compatible tar
set -e
...
# Step 1: Copy rootfs, strip .po
echo "[1/5] Copying files..."
find "$ROOTFS" -type f ! -name "*.po" | while read f; do
    rel="${f#$ROOTFS}"
    rel="${rel#/}"
    dst="$DATA_DIR/$rel"
    mkdir -p "$(dirname "$dst")"
...
# Step 4: Create control.tar.gz
echo "[4/5] Creating control.tar.gz..."
